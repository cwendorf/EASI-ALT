...
alt.testANOVA <- function(...) 
  UseMethod("alt.testANOVA")

alt.testANOVA.default <- function(...){
  data <- data.frame(...)
  columns <- dim(data)[2]
...
alt.testANOVA.formula <- function(formula,...){
  model <- aov(formula,...)
  summary(model)
}
...
alt.testANOVA(Time1,Time2,Time3)

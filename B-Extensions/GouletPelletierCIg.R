...
alt.ciHg <- function(y,conf.level=.95,mu=0,...){
  Var <- alt.ci(y,...)
  n <- as.numeric(Var[1])
  m <- as.numeric(Var[2])
  sd <- as.numeric(Var[3])
...
alt.ciHgMeans <- function(...) 
  UseMethod("alt.alt.ciHgMeans")

alt.ciHgMeans.default <- function(...,conf.level=.95,mu=0){
  data <- data.frame(...)
  results <- data.frame(matrix(ncol=4,nrow=0))
  for (i in 1:ncol(data)) results[i,] <- alt.ciHg(data[,i],conf.level=conf.level,mu=mu)
  colnames(results) <- c("d","g","LL","UL")
  rownames(results) <- colnames(data)
  results
}

alt.ciHgMeans.formula <- function(formula,...) {
  results <- aggregate(formula,FUN=alt.ciHg,...)
  colnames(results) <- c("Group","")
  rownames(results) <- results[,1]
  results[2]
}

alt.ciHgDifference <- function(...) 
  UseMethod("alt.ciHgDifference")  

alt.ciHgDifference.default <- function(...,conf.level=.95){
  Vars <- alt.ciMeans(...)
...
alt.ciHgDifference.formula <- function(formula,conf.level=.95,...){
...
alt.ciHgPairwise <- function(...) 
  UseMethod("alt.ciHgPairwise")
  
alt.ciHgPairwise.default <- function(...,conf.level=.95){
  Vars <- alt.ciMeans(...)
  nr <- dim(Vars)[1]
  rn <- rownames(Vars)
  ncomp <- (nr)*(nr-1)/2
...
alt.ciHgPairwise.formula <- function(formula,conf.level=.95,...){
  Groups <- alt.ciMeans(formula,...)
  nr <- dim(Groups)[1]
  rn <- rownames(Groups)
  ncomp <- (nr)*(nr-1)/2

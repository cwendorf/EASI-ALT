...
alt.ciHSD <- function(formula,conf.level=.95,...){
  model=aov(formula,...)
...
alt.estimateHSD <- function(...) {
  cat("\nCONFIDENCE INTERVALS FOR TUKEY HSD COMPARISONS\n\n")
  print(alt.ciHSD(...)) 
  cat("\n")  
}

alt.nhstHSD <- function(formula,conf.level=.95,...){
  model=aov(formula,...)
...
alt.estimateHSD(Outcome~Group)
alt.testHSD(Outcome~Group)

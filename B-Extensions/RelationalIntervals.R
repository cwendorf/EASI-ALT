...
.ciRelational.formula <- function(formula,conf.level=.95,...){
  results <- .ciMeans(formula,conf.level=conf.level,...)
  mymodel <- aov(formula,...)
...
plotRelational.formula <- function (formula,conf.level=.95,mu=NA,...){
...
estimateRelational(Outcome~Group)
plotRelational(Outcome~Group)

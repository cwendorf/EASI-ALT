...
alt.ciPairwise <- function(...) 
  UseMethod("alt.ciPairwise")

alt.ciPairwise.default <- function(...,conf.level=.95){
  data <- data.frame(...)
  nr <- dim(data)[2]
  rn <- colnames(data)
  ncomp <- (nr)*(nr-1)/2
...
alt.ciPairwise.formula <- function(formula,...){
  varx <- eval(formula[[3]])
  vary <- eval(formula[[2]])
  nr <- nlevels(varx)
  rn <- levels(varx)
...
alt.estimatePairwise <- function(...) {
  cat("\nCONFIDENCE INTERVALS FOR THE PAIRWISE COMPARISONS\n\n")
  print(alt.ciPairwise(...)) 
  cat("\n")  
}

#### NHST Function for Pairwise Comparisons

alt.nhstPairwise <- function(...) 
  UseMethod("alt.nhstPairwise")

alt.nhstPairwise.default <- function(...,conf.level=.95,mu=0){
  data <- data.frame(...)
  nr <- dim(data)[2]
  rn <- colnames(data)
  ncomp <- (nr)*(nr-1)/2
...
alt.nhstPairwise.formula <- function(formula,...){
  varx <- eval(formula[[3]])
  vary <- eval(formula[[2]])
  nr <- nlevels(varx)
  rn <- levels(varx)
...
alt.testPairwise <- function(...) {
  cat("\nHYPOTHESIS TESTS FOR THE PAIRWISE COMPARISONS\n\n")
  print(alt.nhstPairwise(...)) 
  cat("\n")  
}

#### Pairwise Plots

alt.plotPairwise <- function(...) 
  UseMethod("alt.plotPairwise")

alt.plotPairwise.default <- function(...,mu=NULL) {
  main="Confidence Intervals for the Pairwise Comparisons"
  ylab="Mean Difference"
  xlab="Pairwise Comparisons"
  results <- alt.ciPairwise(...)[,c(1,4,5)]
...
alt.plotPairwise.formula <- function(formula,conf.level=.95,mu=NA,...) {
  main="Confidence Intervals for the Pairwise Comparisons"
  ylab="Mean Difference"
  xlab="Pairwise Comparisons"
  results <- alt.ciPairwise(formula,...)[,c(1,4,5)]
  plot(results[,1],xaxt='n',xlim=c(.5,nrow(results)+.5),ylim=c(floor(min(results[,2])/2)*2,ceiling(max(results[,3])/2)*2),xlab=xlab,cex.lab=1.3,ylab=ylab,main=main,las=1,cex=1.5,pch=15,bty="l")
  axis(1,at=1:length(results[,1]),labels=row.names(results))
  for (i in 1:nrow(results)) lines(x=c(i,i), y=c(results[,2][i],results[,3][i]),lwd=2)
  for (i in 1:nrow(results)) text(i,results[,1][i],results[,1][i],cex=.8,pos=2,offset=.5,font=2)
...
alt.estimatePairwise(Outcome~Group)
alt.estimatePairwise(Outcome~Group,conf.level=.99)

alt.plotPairwise(Outcome~Group)
alt.plotPairwise(Outcome~Group,mu=-2,conf.level=.99)

alt.testPairwise(Outcome~Group)
alt.testPairwise(Outcome~Group,mu=-2)
...
alt.estimatePairwise(Time1,Time2,Time3)
alt.estimatePairwise(Time1,Time2,Time3,conf.level=.99)

alt.plotPairwise(Time1,Time2,Time3)
alt.plotPairwise(Time1,Time2,Time3,mu=-2,conf.level=.99)

alt.testPairwise(Time1,Time2,Time3)
alt.testPairwise(Time1,Time2,Time3,mu=-2)

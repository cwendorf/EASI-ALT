...
alt.ciMeans <- function(...) 
  UseMethod("alt.ciMeans")

alt.ciMeans.default <- function(...,conf.level=.95){
  data <- data.frame(...)
  results <- data.frame(matrix(ncol=6,nrow=0))
  for (i in 1:ncol(data)) results[i,] <- alt.ci(data[,i],conf.level=conf.level)
  colnames(results) <- c("N","M","SD","SE","LL","UL")
  rownames(results) <- colnames(data)
  return(results)
}

alt.ciMeans.formula <- function(formula,...) {
  results <- aggregate(formula,FUN=alt.ci,...)
  rn <- results[,1]
  results <- results[[2]]
  rownames(results) <- rn
  return(results)
...
alt.ciDifference <- function(...) 
  UseMethod("alt.ciDifference")

alt.ciDifference.default <- function(x,y,conf.level=.95,...){
...
alt.ciDifference.formula <- function(formula,conf.level=.95,...){
...
alt.estimateMeans <- function(...){
  cat("\nCONFIDENCE INTERVALS FOR THE MEANS\n\n")
  print(alt.ciMeans(...))
  cat("\n")
}

alt.estimateDifference<-function(...) {
  cat("\nCONFIDENCE INTERVAL FOR THE COMPARISON\n\n")
  print(alt.ciDifference(...)) 
  cat("\n")  
}
...
alt.nhstMeans <- function(...) 
  UseMethod("alt.nhstMeans")

alt.nhstMeans.default <- function(...,mu=0){
  data <- data.frame(...)
  results <- data.frame(matrix(ncol=5,nrow=0))
  for (i in 1:ncol(data)) results[i,] <- alt.nhst(data[,i],mu=mu)
  colnames(results) <- c("Diff","SE","t","df","p")
  rownames(results) <- colnames(data)
  results
}

alt.nhstMeans.formula <- function(formula,...) {
  results <- aggregate(formula,FUN=alt.nhst,...)
  rn <- results[,1]
  results <- results[[2]]
  rownames(results) <- rn
  results
...
alt.nhstDifference <- function(...) 
  UseMethod("alt.nhstDifference")

alt.nhstDifference.default <- function(x,y,...){
  model=t.test(x,y,paired=TRUE,...)
  mu=as.numeric(model$null.value) 
  MD=as.numeric(model$estimate)
  SE=as.numeric(model$stderr)
...
alt.nhstDifference.formula <- function(formula,...){
  model=t.test(formula,...)
  mu=as.numeric(model$null.value)
...
alt.testMeans <- function(...){
  cat("\nHYPOTHESIS TESTS FOR THE MEANS\n\n")
  print(alt.nhstMeans(...))
  cat("\n")
}

alt.testDifference <- function(...){
  cat("\nHYPOTHESIS TEST FOR THE COMPARISON\n\n")
  print(alt.nhstDifference(...)) 
  cat("\n")
}
...
alt.cipMeans <- function(results,main,ylab,xlab,mu){
...
alt.cipDifference <- function(results,graph,main,ylab,xlab){
...
alt.plotMeans <- function(...) 
  UseMethod("alt.plotMeans")

alt.plotMeans.default <- function(...,conf.level=.95,mu=NULL){
  main="Confidence Intervals for the Means"
  ylab="Outcome"
  xlab="Variables"
  results <- alt.ciMeans(...,conf.level=conf.level)[,c(2,5,6)]
  alt.cipMeans(results,main,ylab,xlab,mu)
}

alt.plotMeans.formula <- function(formula,mu=NULL,...){
  main="Confidence Intervals for the Means"
  ylab=all.vars(formula)[1]
  xlab=all.vars(formula)[2]
  results <- alt.ciMeans(formula,...)[,c(2,5,6)]
  x <- eval(formula[[3]])
  y <- eval(formula[[2]])
...
alt.plotDifference <- function(...) 
  UseMethod("alt.plotDifference")

alt.plotDifference.default <- function(...){
  main="Confidence Intervals for the Comparison"
  ylab="Outcome"
  xlab="Variables"
  Vars <- alt.ciMeans(...)[2:1,c(2,5,6)]
  Diff <- alt.ciDifference(...)[c(1,4,5)]
  results <- rbind(Vars,Diff)
  Diff <- Diff+Vars[1,1]  
  graph <- rbind(Vars,Diff)
  rownames(graph)[3]="Diff"
  alt.cipDifference(results,graph,main,ylab,xlab)
}

alt.plotDifference.formula <- function(formula,...){
  main="Confidence Intervals for the Comparison"
  ylab=all.vars(formula)[1]
  xlab=all.vars(formula)[2]
  Groups <- alt.ciMeans(formula,...)
  Groups <- Groups[2:1,c(2,5,6)]
  Diff <- alt.ciDifference(formula,...)[c(1,4,5)]
